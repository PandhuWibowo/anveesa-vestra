<template>
  <div class="view-panel">
    ...
    <div v-else class="view-panel__body">
      <div v-if="!links.length" class="empty-state" style="padding:40px 16px">
      ...
      <table v-else class="file-table">
        ...
        <tbody>
          <tr v-for="link in links" :key="link.id" :class="{ 'is-expired': isExpired(link) }">
            <td>
            ...
            <td>
              <span class="base-badge" :class="`base-badge--${link.provider}`">{{ link.provider.toUpperCase() }}</span>
            </td>
            <td>
              {{ link.download_count }}
              <span v-if="link.max_downloads > 0" class="file-type"> / {{ link.max_downloads }}</span>
            </td>
            <td class="file-date">
              <span v-if="link.expires_at">{{ formatDate(link.expires_at) }}</span>
              <span v-else class="file-type">Never</span>
            </td>
            <td class="file-date">{{ formatDate(link.created_at) }}</td>
            <td class="file-actions" style="white-space:nowrap">
              <button class="row-btn" style="opacity:1" @click="copyLink(link.token)" title="Copy link">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
                </svg>
              </button>
              <button class="row-btn danger" style="opacity:1" @click="handleRevoke(link.id)" title="Revoke link">
